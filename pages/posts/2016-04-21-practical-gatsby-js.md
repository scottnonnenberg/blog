...
Even sadder, the world of scrolling in SPAs is pretty nasty. Browsers have some interesting  behavior around deep links (regarding back/forward behavior and delayed asset loading), and today a SPA needs to try to replicate all that. I've tried this code in `wrappers/md.js`, but I'm not really satisfied with it.
...
You'll need to include `bundle.js` in `develop` mode because there's no server rendering, but not in `build` mode. You won't want `styles.css` in `develop` mode, but you'll need it in `build` mode because [Webpack](https://webpack.github.io/) won't be there to inject the styles.
